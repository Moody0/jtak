...
import 'package:app_jtak_warehouse/main.dart';
...
// Compatibility names used throughout the legacy merchant UI. Keeping the
// mapping here avoids visual regressions while using Flutter's current API.
extension LegacyTextThemeCompatibility on TextTheme {
...
extension NavigationExtension on BuildContext {
  ...
  void popUntil(String path, {Object? data}) =>
      navigation.popUntil(ModalRoute.withName(path));
  ...
  Future<T?> navigateToReset<T>(String path,
      {String untilPath = MyApp.routeName, Object? data}) async {
    return await Navigator.of(this).pushNamedAndRemoveUntil(
        path, ModalRoute.withName(untilPath),
        arguments: data);
  }
...
extension ToastExtention on BuildContext {
  void showSnakBar(String? text, {int? milliseconds}) =>
